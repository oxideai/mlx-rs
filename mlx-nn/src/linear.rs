...
use crate::{macros::ModuleParameters, module::{Param, Module}};
...
impl Linear {
    pub fn new(input_dims: i32, output_dims: i32, with_bias: impl Into<WithBias>) -> Result<Self, Exception> {
        let scale = f32::sqrt(1.0 / (input_dims as f32));
        let weight = mlx_rs::random::uniform::<_, f32>(-scale, scale, &[output_dims, input_dims], None)?;

        let bias = match with_bias.into() {
            WithBias::Yes => {
                Some(mlx_rs::random::uniform::<_, f32>(-scale, scale, &[output_dims], None)?)
            }
            WithBias::No => None,
        };
...
impl Bilinear {
    pub fn new(input_dims_1: i32, input_dims_2: i32, output_dims: i32, with_bias: impl Into<WithBias>) -> Result<Self, Exception> {
        let scale = f32::sqrt(1.0 / (input_dims_1 as f32));
        let weights = mlx_rs::random::uniform::<_, f32>(-scale, scale, &[output_dims, input_dims_2, input_dims_1], None)?;
