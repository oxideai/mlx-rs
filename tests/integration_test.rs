...
fn test_equal_op() {
    ...
    assert_eq!(result.all(None).item::<bool>(), false);
}
...
fn test_not_equal_op() {
    ...
    assert_eq!(result.shape(), &[4, 3]);
    assert_eq!(result.dtype(), Dtype::Bool);
    assert_eq!(result.all(None).item::<bool>(), true);
}
...
fn test_all2() {
    ...
    assert_eq!(a.dtype(), Dtype::Float32);
    let result = a.all_axes(&[0, -1], None);
