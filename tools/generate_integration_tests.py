...
def verify_array(indent, name: str, array: mx.array) -> str:
    ...
    if dtype == mx.bool_:
        ...
        result += assert_equal(
            indent, f"{name}.all(None).item::<bool>()", "true" if all else "false"
        )
...
def generate_integration_tests():
    with open("../tests/integration_test.rs", "w") as f:
        ...
        array_only_functions = [
            ...
            dict(name="all", array_only=True, axis=True, axes=True, rust_extra="None"),
